...
class HistorialAbonosPage extends StatefulWidget {
  HistorialAbonosPage(this.compra_id, this.nombres);
  final String compra_id;
  final String nombres;
  @override
  _HistorialAbonosPageState createState() => _HistorialAbonosPageState();
}

class _HistorialAbonosPageState extends State<HistorialAbonosPage> {
  ...
  void initState() {
    ...
    _getDataStartTimeout();
  }

  _getDataStartTimeout() async {
    // 1seg son 1000ms
    ...
    return new Scaffold(
      ...
      bottomNavigationBar: FABBottomAppBar(
        ...
        items: [
          ...
          FABBottomAppBarItem(
              text2: "Debe pagar",
              text: "\$" + total_obligacion.toString(),
              is_numeric: true),
          FABBottomAppBarItem(
              text2: "Abonos ",
              text: "\$" + total_valor.toString(),
              is_numeric: true),
          FABBottomAppBarItem(
            text2: "Saldo ",
  ...
  _onPressedEliminarAbonos() async {
    ...
    if (_selectedItemModel != null) {
      _ShowDialogProgress();
      var body = json.encode({"cobro": _session_grupo.toString(), "id": id});
                      ...
                      children: <Widget>[
                        ...
                        Text(
                          "Tu obligacion mensual es \$" +
                              total_obligacion_mensual.toString(),
  ...
  Widget _rightSectionValues(List<AbonosModel> model, String index) {
    ...
    return Container(
      child: new Column(
        ...
        children: <Widget>[
          new Text(
            "\$" + data.valor_prestamo_anterior.toString(),
            style: new TextStyle(
                color: Colors.redAccent,
                fontSize: 14.0,
                decoration:
                    (double.parse(data.valor_prestamo_anterior.toString()) ==
                            double.parse(data.valor_prestamo_actual.toString()))
                        ? TextDecoration.none
                        : TextDecoration.lineThrough),
          ),
          new Text(
            "\$" + data.valor.toString(),
            style: new TextStyle(color: Colors.lightGreen, fontSize: 14.0),
          ),
        ],
      ),
    );
